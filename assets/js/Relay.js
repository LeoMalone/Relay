...
// ----------------------------------------------- FUNCTIONS -----------------------------------------------
function buttonChange(b_id, c_id) {        // this is where the logic starts on button click
    if (currentBtn) {
...
function buildOptionsCheck(element) {
    if (buildQ2Card) {
...
$("#optim_card").validate({
    rules: {
        ...
        optim_other_ta: {
            required: function (element) {
                return $("#optimize_q3").val().includes("0");
            },
            minlength: 1,
            maxlength: 1000
        }
